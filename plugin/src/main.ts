...
async function createShare(
	...
	let hyphenated_title = (share.title.split("/").last() || "").replace(
		/[^a-zA-Z0-9]/g,
		"-",
	);
				...
				if (cache.embeds) {
					for (const embed of cache.embeds) {
						...
						content = content.replace(
							embed.original,
							`![${embed.displayText}](${this.settings.serverUri}/api/##id##/${urlencoded})`,
						);
					}
				}
...
class RecursiveShareSettingsTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			...
			.setDesc(
				"You'll need a hosted instance of Recursive Share to use this plugin",
			)
			.addText((text) =>
				text
